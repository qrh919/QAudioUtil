...
NSString *const kQAudioDataKey;

@interface QAVAudioPlayer ()
<
...
- (NSOperationQueue *)audioDataOperationQueue {
    if (_audioDataOperationQueue == nil) {
        ...
        audioDataOperationQueue.name = @"com.qrh.AVAudipPlayer.loadAudioDataQueue";
...
- (void)playAudioWithURLString:(NSString *)URLString identifier:(NSString *)identifier {
    ...
    if ([self.URLString isEqualToString:URLString] && self.identifier == identifier) {
        ...
        [self setAudioPlayerState:QVoiceMessagePlayState_Cancel];
        return;
    }
    ...
    NSBlockOperation *blockOperation = [NSBlockOperation blockOperationWithBlock:^{
        [self setAudioPlayerState:QVoiceMessagePlayState_Downloading];
    ...
    [blockOperation setName:[[NSString stringWithFormat:@"%@_%@",self.URLString, self.identifier] MD5String]];
...
- (NSData *)audioDataFromURLString:(NSString *)URLString identifier:(NSString *)identifier {
    ...
    if (audioData) {
        objc_setAssociatedObject(audioData, &kQAudioDataKey, [[NSString stringWithFormat:@"%@_%@",URLString,identifier] MD5String], OBJC_ASSOCIATION_COPY);
    }
...
- (void)playAudioWithData:(NSData *)audioData {
    ...
    NSString *audioURLMD5String = objc_getAssociatedObject(audioData, &kQAudioDataKey);
    
    if (![[[NSString stringWithFormat:@"%@_%@",self.URLString,self.identifier] MD5String] isEqualToString:audioURLMD5String]) {
        return;
    }
    ...
    [self setAudioPlayerState:QVoiceMessagePlayState_Playing];
    [_audioPlayer play];
}
...
- (void)cancelOperation {
    for (NSOperation *operation in self.audioDataOperationQueue.operations) {
        if ([operation.name isEqualToString:[[NSString stringWithFormat:@"%@_%@",self.URLString, self.identifier] MD5String]]) {
            [operation cancel];
            break;
        }
    }
...
- (void)audioPlayerDidFinishPlaying:(AVAudioPlayer *)player successfully:(BOOL)flag {
    [self setAudioPlayerState:QVoiceMessagePlayState_End];
    //    //删除近距离事件监听
    //    [[UIDevice currentDevice] setProximityMonitoringEnabled:NO];
...
- (NSString *)cachePath {
    if (!_cachePath) {
        _cachePath = [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) firstObject] stringByAppendingPathComponent:@"com.mzy.chat.audioCache"];
...
- (void)setAudioPlayerState:(QVoiceMessagePlayState)audioPlayerState {
    _audioPlayerState = audioPlayerState;
    if (_audioPlayerState == QVoiceMessagePlayState_Cancel ||
        _audioPlayerState == QVoiceMessagePlayState_Normal) {
        _URLString = nil;
        _identifier = nil;
    }
}
